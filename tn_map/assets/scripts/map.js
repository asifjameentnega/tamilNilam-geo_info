...
function mapclick(evt, latitude = null, longitude = null) {
  ...
  $.ajax(settings).done(function (response) {
    ...
    if (adminresponseData.data.rural_urban == 'rural') {
      ...
      $('#type').text('(District | Taluk | Village)');
      $('#value').text(`${response.data.district_name} | ${response.data.taluk_name} | ${response.data.village_name}`);
      localStorage.setItem('taluk_name', response.data[name]);
  ...
  $('#fmb_trigger').on('click', function (e) {
    ...
    $.ajax({
      ...
      data: {
        ...
        area_type: adminresponseData.data.rural_urban.toLowerCase()
      },
      beforeSend: function () {
        showSpinner("Fetching data from NIC... Please wait...!");
      },
...
$(document).on('click', '#trigger', function (e) {
  ...
  if (adminresponseData.data.rural_urban == 'rural') {
    ...
    data = {
      district_code: responseData.data.district_code.toString().padStart(2, '0'),
      taluk_code: responseData.data.taluk_code.toString().padStart(2, '0'),
      town_code: responseData.data.revenue_town_code.toString().padStart(2, '0'),
...
function callTriggerFunction() {
  ...
  if (responseData && responseData.success != 2) {
    ...
    land_type = adminresponseData.data.rural_urban.toLowerCase();
    ...
    if (land_type === "rural") {
      ...
      specificData = {
        town_code: responseData.data.revenue_town_code.toString().padStart(3, '0'),
