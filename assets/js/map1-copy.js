...
function shapetojson_raw_fmb(district_val,taluk_val,village_val,survey_number)
{
    ...
    var full_path = app_temp_path+'fmb_raw/'+district_val+'/'+taluk_val+'/'+village_val+'/fmb/raw/'+survey_number+'.zip';
    $.ajax({
        url:full_path,
        type:'HEAD',
        error: function()
...
function shapetojson_georef_fmb(district_val,taluk_val,district_text,taluk_text,village_text,village_val,village_val_excluding_zero,georef_vector_file)
{
    if (goeref_fmbgeojsonLayer != undefined) {
        georef_map1.removeLayer(goeref_fmbgeojsonLayer);
        $('.geojson_label').text('');
